"""Tests for the Squadcast alert handler."""
...
_WEBHOOK_URL = "https://api.squadcast.com/v2/incidents/api/abc123"
...
def _make_alert(level: AlertLevel = AlertLevel.CRITICAL) -> Alert:
    return Alert(
        ...
        expected_run_at=datetime(2024, 1, 10, 3, 0, tzinfo=timezone.utc),
    )


@pytest.fixture()
def handler() -> SquadcastAlertHandler:
    return SquadcastAlertHandler(webhook_url=_WEBHOOK_URL)
...
class TestSquadcastAlertHandler:
    ...
    def test_payload_contains_job_name(self, handler, mock_urlopen):
        ...
        assert "nightly-backup" in body["message"] or body["tags"]["job"] == "nightly-backup"

    def test_critical_maps_to_trigger(self, handler, mock_urlopen):
        handler.send(_make_alert(AlertLevel.CRITICAL))
        req = mock_urlopen.call_args[0][0]
        body = json.loads(req.data)
        assert body["status"] == "trigger"

    def test_warning_maps_to_trigger(self, handler, mock_urlopen):
        handler.send(_make_alert(AlertLevel.WARNING))
        req = mock_urlopen.call_args[0][0]
        body = json.loads(req.data)
        assert body["status"] == "trigger"

    def test_info_maps_to_resolve(self, handler, mock_urlopen):
    ...
    def test_url_error_raises_runtime_error(self, handler):
        ...
        with patch(
            ...
            with pytest.raises(RuntimeError, match="Squadcast"):
                handler.send(_make_alert())
